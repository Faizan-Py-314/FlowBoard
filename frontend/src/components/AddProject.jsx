  ...
  return (
    <div onClick={handleBackdropClick} className={`fixed inset-0 h-svh flex items-center justify-between z-30 transition-opacity duration-150 ${isVisible ? 'opacity-100 pointer-events-auto' : 'opacity-0 pointer-events-none'}`}>
      ...
      <h2 className='text-2xl font-bold ml-2'>{isEditMode? 'Edit Project Details':'Add New Project'}</h2>
        ...
        <div>
          <span className={` ${isError == 'NameEmptyError'? 'block':'hidden'} text-xs text-red-500 mx-2`}>This Field Should not be Empty</span>
          <input onChange={handleChange} className={`${isError == 'NameEmptyError'? 'border-red-500':'border-gray-300'} focus:outline-none border py-2 px-3 w-full rounded-md`} type="text" name='name' value={formData.name} placeholder='Project Name' />
        </div>
        <div>
          <span className={` ${isError == 'DescriptionEmptyError'? 'block':'hidden'} text-xs text-red-500 mx-2`}>This Field Should not be Empty</span>
          <textarea onChange={handleChange} className={`${isError == 'DescriptionEmptyError'? 'border-red-500':'border-gray-300'} focus:outline-none border border-gray-300 py-2 px-3 w-full rounded-md`} rows='4' type="text" name='description' value={formData.description} placeholder='Project Description'></textarea>
        </div>
        ...
        <div className='p-3 border border-gray-400 rounded-md flex flex-col gap-1 pb-2 md:gap-2'>
          ...
          <div ref={containerRef} className='flex flex-col gap-2 h-21.5 overflow-auto no_scrollbar md:h-23'>
            {requirements.map((requirement, index) => {
                return <input required onChange={(e) => {handleChangeRequirements(e, index)}} key={index} value={requirement} className=' focus:outline-none border border-gray-300 py-2 px-3 w-full rounded-md' type="text" placeholder={`Requirement ${index+1}`} />
            })}
          </div>
        ...
        <button type='submit' className='w-full p-2 bg-black text-white rounded-md cursor-pointer'>{isEditMode? 'Update Project':'Create Project'}</button>
      </form>
    </div>
  )
}
